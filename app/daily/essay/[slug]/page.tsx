...
export async function generateMetadata({
  ...
  const post = await fetchLaborPostByRequiredWpCategorySlug(decodedSlug, "essay")
  if (!post) return { title: "文章" }

  const siteOrigin = await getSiteOrigin()
  const canonical = `${siteOrigin}/daily/essay/${encodeURIComponent(decodedSlug)}`
...
export default async function DailyEssaySlugPage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
